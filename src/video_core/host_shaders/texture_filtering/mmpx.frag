...
void main()
{
    ...
    #define src(x, y) (checkp(x, y) ? texture(tex,coord + vec2(x, y) * inv_source_size) : vec4(0.0))

////////////////////////////////////////////////////////////////////////////////
	vec4 E = texture(tex, coord);
...
if (!skiprest) {
	...
	vec4 PC = src(+1.0, -2.0);
	vec4 QA = src(-2.0, -1.0);
	vec4 QG = src(-2.0, +1.0); //             AA    PA    [P]   PC    CC
	vec4 RC = src(+2.0, -1.0); //                ┌──┬──┬──┐
	vec4 RI = src(+2.0, +1.0); //             QA │  A │  B │ C  │ RC
	vec4 SG = src(-1.0, +2.0); //                ├──┼──┼──┤
	vec4 SI = src(+1.0, +2.0); //            [Q] │  D │  E │ F  │ [R]
	vec4 AA = src(-2.0, -2.0); //                ├──┼──┼──┤
	vec4 CC = src(+2.0, -2.0); //             QG │  G │  H │ I  │ RI
	vec4 GG = src(-2.0, +2.0); //                └──┴──┴──┘
	vec4 II = src(+2.0, +2.0); //             GG    SG    [S]   SI    II
