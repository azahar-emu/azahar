...
bool RasterizerCache<T>::ValidateByReinterpretation(Surface& surface, SurfaceParams params,
    ...
    //
    // Do not skip validation if an Invalid-format surface overlaps the interval.
    // Such a surface indicates that the region was cleared, so cached contents must
    // not be reused through the stride-aliasing fast path.
    bool has_invalid = false;
    const PAddr region_addr = boost::icl::lower(interval);
    const u32 region_size = boost::icl::length(interval);

    ForEachSurfaceInRegion(region_addr, region_size,
                           [&](SurfaceId /*surface_id*/, Surface& cached_surface) {
    ...
    // Only skip validation when the entire interval is owned by a GPU-modified
    // surface. An overlapping dirty region is not sufficient to prove that the
    // complete interval can safely reuse the cached surface.
    if (has_invalid || !boost::icl::contains(dirty_regions, interval)) {
        return false;
    }
